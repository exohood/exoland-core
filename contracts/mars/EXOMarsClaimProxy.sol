...
import "../interface/EXORuleProxy.sol";
import "../interface/MarsClaim.sol";

contract EXOMarsClaimProxy is EXORuleProxy{
    ...
    MARSClaim public _MarsClaim = MarsClaim(0x0);
    struct RuleData{
    ...
    EXOFactoryV2 public _factory = EXOFactoryV2(0x0);
        ...
        public 
    {
        ...
        _MarsClaim = MarsClaim(MarsClaim);
    ...
    function destroy(  address owner, IGegoToken.Gego calldata gego) external {
        ...
        _MarsClaim.burnWithdrawToken(exo.erc20, owner, exo.amount);
    } 


    function generate(address user , uint256 ruleId ) external view returns (EXOToken.exo memory exo ){
        require( _factory == EXOFactoryV2(msg.sender) ," invalid factory caller" );
        require(_ruleSwitch[ruleId], " rule is closed ");
        require(false, "can't mint");
        user;
        EXOToken.exo memory _exo;
        return _exo;
    } 
